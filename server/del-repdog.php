...
$colname_rs_delr = "-1";
if (isset($_GET['rId'])) {
  $colname_rs_delr = $_GET['rId'];
}

$query_rs_delr = sprintf("SELECT rName, rPicture FROM repdog WHERE rId = %s", GetSQLValueString($Wandering,$colname_rs_delr, "int"));
$rs_delr = mysqli_query($Wandering, $query_rs_delr) or die(mysqli_error());
$row_rs_delr = mysqli_fetch_assoc($rs_delr);
$totalRows_rs_delr = mysqli_num_rows($rs_delr);




if ((isset($_GET['rId'])) && ($_GET['rId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM repdog WHERE rId=%s",
 ...
 $updateGoTo = "show-repdog.php";
  ...
  @unlink("../images/repdog/" . $row_rs_delr['rPicture']);
  
header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功刪除代表狗狗→" . $row_rs_delr['rName'] . "成功')</script>"; 

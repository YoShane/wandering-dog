...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE webfaq SET wQuestion=%s, wAnswer=%s WHERE wId=%s",
                       GetSQLValueString($Wandering,$_POST['wQuestion'], "text"),
                       GetSQLValueString($Wandering,$_POST['wAnswer'], "text"),
 ...
 header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功編輯問答→" . $_POST['wQuestion'] . "成功')</script>"; 
echo "<script>document.location.href='" . $updateGoTo . "';</script>";
}

$colname_rs_edwebfaq = "-1";
if (isset($_GET['wId'])) {
  $colname_rs_edwebfaq = $_GET['wId'];
}

$query_rs_edwebfaq = sprintf("SELECT * FROM webfaq WHERE wId = %s", GetSQLValueString($Wandering,$colname_rs_edwebfaq, "int"));
$rs_edwebfaq = mysqli_query($Wandering, $query_rs_edwebfaq) or die(mysqli_error());
$row_rs_edwebfaq = mysqli_fetch_assoc($rs_edwebfaq);
$totalRows_rs_edwebfaq = mysqli_num_rows($rs_edwebfaq);
...
================================================== -->
      ...
      <div class="container">
        ...
        <div class="right-side">
          ...
          <div class="header-widget">
					...
					<a href="add-webfaq.php" class="button border with-icon">編輯網站問答<i class="sl sl-icon-plus"></i></a>
				</div>
          <!-- Header Widget / End -->
        </div>
        <!-- Right Side Content / End -->
      ...
      <ul data-submenu-title="網站內容">
        ...
        <li  class="active"><a> <i class="sl sl-icon-ghost"></i> 網站說明</a>
          <ul>
            ...
            <li  class="active"><a href="show-webfaq.php">網站問與答</a></li>
          </ul>
        </li>
          ...
          <div id="add-listing">
            ...
            <div class="add-listing-section">
              ...
              <div class="add-listing-headline">
                <h3><i class="sl sl-icon-note"></i>輸入問答內容</h3>
              </div>
              <!-- Title -->
              <div class="row with-forms">
                <div class="col-md-12">
                  <h5>問題內容</h5>
                  <input name="wQuestion" type="text" required  class="search-field" placeholder="輸入問題" value="<?php echo $row_rs_edwebfaq['wQuestion']; ?>" />
                </div>
              </div>
              <!-- Row -->
              <div class="row with-forms">
 <div class="col-md-12">
                  <h5>回覆內容</h5>
                  <textarea name="wAnswer" required class="search-field" id="fAnswer"><?php echo $row_rs_edwebfaq['wAnswer']; ?></textarea>
</div>
...
<?php
mysqli_free_result($rs_edwebfaq);
?>

...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE slider SET sOrder=%s, sPicture=%s, sPosition=%s, sTitle=%s, sContent=%s, sScale=%s, sButtonname=%s, sButtonlink=%s WHERE sId=%s",
                       GetSQLValueString($Wandering,$_POST['sOrder'], "int"),
                       GetSQLValueString($Wandering,$_name_, "text"),
                       GetSQLValueString($Wandering,$_POST['sPosition'], "int"),
                       GetSQLValueString($Wandering,$_POST['sTitle'], "text"),
                       GetSQLValueString($Wandering,nl2br($_POST['sContent']), "text"),
                       GetSQLValueString($Wandering,$_POST['sScale'], "int"),
                       GetSQLValueString($Wandering,$_POST['sButtonname'], "text"),
                       GetSQLValueString($Wandering,$_POST['sButtonlink'], "text"),
                       GetSQLValueString($Wandering,$_GET['sId'], "int"));

  
  $Result1 = mysqli_query($Wandering, $updateSQL) or die(mysqli_error());

  $updateGoTo = "show-slider.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    ...
    header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功編輯滑動版面→" . $_POST['sTitle'] . "成功')</script>"; 
echo "<script>document.location.href='" . $updateGoTo . "';</script>";
}

$colname_ed_slider = "-1";
if (isset($_GET['sId'])) {
  $colname_ed_slider = $_GET['sId'];
}

$query_ed_slider = sprintf("SELECT * FROM slider WHERE sId = %s", GetSQLValueString($Wandering,$colname_ed_slider, "int"));
$ed_slider = mysqli_query($Wandering, $query_ed_slider) or die(mysqli_error());
$row_ed_slider = mysqli_fetch_assoc($ed_slider);
$totalRows_ed_slider = mysqli_num_rows($ed_slider);
?>
...
================================================== -->
      ...
      <ul data-submenu-title="網站內容">
        ...
        <li><a> <i class="sl sl-icon-ghost"></i> 網站說明</a>
         ...
         <li  class="active"><a> <i class="sl sl-icon-paper-plane"></i> 網站內容設定</a>
          <ul>
            <li  class="active"><a href="show-slider.php">首頁滑動版面</a></li>
          ...
          <div id="add-listing">
            ...
            <div class="add-listing-section">
              ...
              <div class="add-listing-headline">
                <h3><i class="sl sl-icon-note"></i>滑動版面內容</h3>
              </div>
              <!-- Title -->
              <div class="row with-forms">
                <div class="col-md-12">
                  <h5>標題</h5>
                  <input name="sTitle" type="text" required  class="search-field" placeholder="輸入標題" value="<?php echo $row_ed_slider['sTitle']; ?>" />
                </div>
              </div>
              <!-- Row -->
              <div class="row with-forms">
 <div class="col-md-12">
                  <h5>內容</h5>
                  <textarea name="sContent" required class="search-field" id="sContent"><?php echo br2nl($row_ed_slider['sContent']); ?></textarea>
</div>
          ...
          <!-- Headline -->
          <div class="add-listing-headline">
            <h3><i class="sl sl-icon-picture"></i>代表照片(建議大小1733*1300)</h3>
            <input name="Filename" type="hidden" id="Filename" value="<?php echo $row_ed_slider['sPicture']; ?>">
          </div>
          <!-- Dropzone -->
          <div class="submit-section">
             <div class="photoUpload">
							    <span><i class="sl sl-icon-arrow-up-circle"></i> 上傳封面照片</span>
             ...
             <div class="add-listing-headline margin-top-15">
                <h3><i class="sl sl-icon-settings"></i>滑動版面設定</h3>
              </div>
              
               <div class="row with-forms">
                <div class="col-md-4">
									<h5>文字位置</h5>
									<select name="sPosition" class="chosen-select-no-single" id="s" >
									  <option value="1" <?php if (!(strcmp(1, $row_ed_slider['sPosition']))) {echo "selected=\"selected\"";} ?>>靠左</option>
									  <option value="2" <?php if (!(strcmp(2, $row_ed_slider['sPosition']))) {echo "selected=\"selected\"";} ?>>置中</option>
									  <option value="3" <?php if (!(strcmp(3, $row_ed_slider['sPosition']))) {echo "selected=\"selected\"";} ?>>靠右</option>
                                    </select>
				  </div>
                                
                  <div class="col-md-4">
					<h5>排序<span>(選填)</span><i class="tip" data-tip-content="數字越小，優先度越高。"></i></h5>
							  <input name="sOrder" type="number" required="required" id="sOrder" min="0" value="<?php echo $row_ed_slider['sOrder']; ?>">
						 </div>
                            
                  <div class="col-md-4">
					<h5>最大縮放<i class="tip" data-tip-content="本縮放代表使用者在瀏覽過程，滑動圖片放大的比例(有設定會比較有動態美，設置範圍建議100-110，可以設定完看看效果)"></i></h5>
					  <input name="sScale" type="number" required="required" id="sScale" min="100" value="<?php echo $row_ed_slider['sScale']; ?>">
					</div>           
              ...
              <div class="col-md-6">
					<h5>按鈕文字</h5>
									<input name="sButtonname" type="text" required="required" id="sButtonname" value="<?php echo $row_ed_slider['sButtonname']; ?>">
								</div>
                                
                  <div class="col-md-6">
					<h5>按鈕連結</h5>
					  <input name="sButtonlink" type="text" required="required" id="sButtonline" value="<?php echo $row_ed_slider['sButtonlink']; ?>">
					</div>

...
$colname_rs_repdog = "-1";
if (isset($_GET['dId'])) {
  $colname_rs_repdog = $_GET['dId'];
}

$query_rs_repdog = sprintf("SELECT * FROM repdog WHERE rLdid = %s ORDER BY rOrder ASC", GetSQLValueString($Wandering,$colname_rs_repdog, "int"));
$rs_repdog = mysqli_query($Wandering, $query_rs_repdog) or die(mysqli_error());
$row_rs_repdog = mysqli_fetch_assoc($rs_repdog);
$totalRows_rs_repdog = mysqli_num_rows($rs_repdog);
		...
		<div class="container">
			...
			<div class="right-side">
				...
				<div class="header-widget">
					...
					<a href="add-repdog.php?dId=<?php echo $_GET['dId']; ?>" class="button border with-icon">新增代表狗狗<i class="sl sl-icon-plus"></i></a>
				</div>
				<!-- Header Widget / End -->
			</div>
			<!-- Right Side Content / End -->
		...
		<div class="row">
			...
			<div class="col-lg-12 col-md-12">
				<div class="dashboard-list-box margin-top-0">
                    ...
                    <?php if($totalRows_rs_repdog != 0){ ?>
					  <?php do { ?>
					   	<li>
							<div class="list-box-listing">
								<div class="list-box-listing-img"><a href="#"><img src="../images/repdog/<?php echo $row_rs_repdog['rPicture']; ?>" alt=""></a></div>
								<div class="list-box-listing-content">
									<div class="inner">
										<h3><a href="#"><?php echo $row_rs_repdog['rName']; ?></a></h3>
										<span>Gender：<?php if($row_rs_repdog['rGender'] == 0){ echo "♀Female";}elseif($row_rs_repdog['rGender'] == 1){ echo "♂Male";}else { echo "Intersex";}?> ；Age：<?php echo $row_rs_repdog['rAge']; ?> </span>
										<br><span>優先排序：<?php echo $row_rs_repdog['rOrder']; ?> (更新:<?php echo date('Y-m-d 時間H:i',strtotime($row_rs_repdog['rEtime'])); ?>)</span>
																			
									</div>
								</div>
							</div>
							<div class="buttons-to-right">
                           	<a href="edit-repdog.php?rId=<?php echo $row_rs_repdog['rId']; ?>&dId=<?php echo $_GET['dId']; ?>" class="button gray"><i class="sl sl-icon-note"></i> 編輯</a>
								<a href="del-repdog.php?rId=<?php echo $row_rs_repdog['rId']; ?>&dId=<?php echo $_GET['dId']; ?>" class="button gray"  onclick="javascript:return confirm('確定刪除代表狗狗「<?php echo $row_rs_repdog['rName']; ?>」？')" ><i class="sl sl-icon-close"></i> 刪除</a>
							</div>
						</li>
                        <?php } while ($row_rs_repdog = mysqli_fetch_assoc($rs_repdog)); ?>
                        <?php }else{ ?>
                       
                      <h1><center> 還沒有設定代表狗狗喔~ </center></h1>

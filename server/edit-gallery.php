...
define("DESTINATION_FOLDER", "../images/gallerylist");
define("no_error", "");
define("yes_error", "");
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE listgallery SET lName=%s, lOrder=%s, lPicture=%s WHERE lId=%s",
                       GetSQLValueString($Wandering,$_POST['lName'], "text"),
                       GetSQLValueString($Wandering,$_POST['lOrder'], "int"),
                       GetSQLValueString($Wandering,$_name_, "text"),
                       GetSQLValueString($Wandering,$_GET['lId'], "int"));

  
  $Result1 = mysqli_query($Wandering, $updateSQL) or die(mysqli_error());

  $updateGoTo = "show-gallery.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功編輯相簿類型→" . $_POST['lName'] . "成功')</script>"; 
echo "<script>document.location.href='" . $updateGoTo . "';</script>";
}

$colname_rs_edgallery = "-1";
if (isset($_GET['lId'])) {
  $colname_rs_edgallery = $_GET['lId'];
}

$query_rs_edgallery = sprintf("SELECT * FROM listgallery WHERE lId = %s", GetSQLValueString($Wandering,$colname_rs_edgallery, "int"));
$rs_edgallery = mysqli_query($Wandering, $query_rs_edgallery) or die(mysqli_error());
$row_rs_edgallery = mysqli_fetch_assoc($rs_edgallery);
$totalRows_rs_edgallery = mysqli_num_rows($rs_edgallery);
?>
    ...
    <form action="<?php echo $editFormAction; ?>"  name="form1" id="form1" method="POST" enctype="multipart/form-data">
      <div class="row">
        <div class="col-lg-12">
          <div id="add-listing">
            ...
            <div class="add-listing-section">
              ...
              <div class="add-listing-headline">
                <h3><i class="sl sl-icon-note"></i>基本設定</h3>
              </div>
              <!-- Title -->
              <div class="row with-forms">
                <div class="col-md-8">
                  <h5>相簿名稱</h5>
                  <input name="lName" type="text" required  class="search-field" placeholder="輸入相簿名稱" value="<?php echo $row_rs_edgallery['lName']; ?>" />
                </div>
                <div class="col-md-4">
                  <h5>相簿排序(選填，預設100放後面)<i class="tip" data-tip-content="狗場分類將會互相聯繫，此處可設定放在狗場項目前(0-49)或項目後(50-100)，排序數字越小則越優先，預設100放後面"></i></h5>
                  <input name="lOrder" type="number" required  class="search-field" placeholder="輸入排序號碼" max="100" min="0" value="<?php echo $row_rs_edgallery['lOrder']; ?>" />
                </div>
              </div>
          ...
          <!-- Headline -->
          <div class="add-listing-headline">
            <h3><i class="sl sl-icon-picture"></i>代表照片</h3>
          </div>
          <!-- Dropzone -->
          <div class="submit-section">
             <div class="photoUpload">
                                ...
                                <input name="Filename" type="hidden" id="Filename" value="<?php echo $row_rs_edgallery['lPicture']; ?>">
                             							</div>
          </div>
...
<?php
mysqli_free_result($rs_edgallery);

mysqli_free_result($rs_showphoto);

mysqli_free_result($rs_pointphoto);

mysqli_free_result($rs_galleryphotophoto);
?>

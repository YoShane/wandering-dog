...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE dogpoint SET dType=%s, dName=%s, dHavedog=%s, dIntroduction=%s, dPicture=%s, dManagement=%s, dHphone=%s, dCphone=%s, dMap=%s, dMappoint=%s, dYoutube=%s, dFb=%s WHERE dId=%s",
...
//清除需求重設
 ...
 $deleteSQL = sprintf("DELETE FROM dogneed WHERE nLdId=%s",
                       GetSQLValueString($Wandering,$_GET['dId'], "int"));
...
mysqli_free_result($rs_findid);
  ...
  header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功編輯" . $_POST['dName'] . "成功')</script>"; 
echo "<script>document.location.href='./show-dogpoint.php';</script>";
}

$colname_rs_findpoint = "-1";
if (isset($_GET['dId'])) {
  $colname_rs_findpoint = $_GET['dId'];
}

$query_rs_findpoint = sprintf("SELECT * FROM dogpoint WHERE dId = %s", GetSQLValueString($Wandering,$colname_rs_findpoint, "int"));
...
$colname_rs_basicneed = "-1";
if (isset($_GET['dId'])) {
  $colname_rs_basicneed = $_GET['dId'];
}

$query_rs_basicneed = sprintf("SELECT * FROM dogneed WHERE nLdid = %s AND nLtid > 1 ORDER BY nId ASC", GetSQLValueString($Wandering,$colname_rs_basicneed, "int"));
$rs_basicneed = mysqli_query($Wandering, $query_rs_basicneed) or die(mysqli_error());
$row_rs_basicneed = mysqli_fetch_assoc($rs_basicneed);
$totalRows_rs_basicneed = mysqli_num_rows($rs_basicneed);

$colname_rs_othersneed = "-1";
if (isset($_GET['dId'])) {
  $colname_rs_othersneed = $_GET['dId'];
}

$query_rs_othersneed = sprintf("SELECT * FROM dogneed WHERE nLdid = %s AND nLtid = 1 ORDER BY nId ASC", GetSQLValueString($Wandering,$colname_rs_othersneed, "int"));
$rs_othersneed = mysqli_query($Wandering, $query_rs_othersneed) or die(mysqli_error());
$row_rs_othersneed = mysqli_fetch_assoc($rs_othersneed);
$totalRows_rs_othersneed = mysqli_num_rows($rs_othersneed);
?>
...
<!-- Main Navigation -->
          ...
          <div class="row">
            <div class="col-lg-12">
              ...
              <div id="add-listing">
                ...
                <div class="add-listing-section">
                  ...
                  <div class="row with-forms">
                    <div class="col-md-12">
                      <h5>狗場名稱</h5>
                      <input name="dName" type="text" required  class="search-field" placeholder="請在此輸入狗場名稱" value="<?php echo $row_rs_findpoint['dName']; ?>" />
                      </div>
                    </div>
                  ...
                  <div class="row with-forms">
                    ...
                    <div class="col-md-6">
                      ...
                      <select name="dType" class="chosen-select-no-single" >
                        <option value="1" <?php if (!(strcmp(1, $row_rs_findpoint['dType']))) {echo "selected=\"selected\"";} ?>>一般狗場</option>
                        <option value="1000" <?php if (!(strcmp(1000, $row_rs_findpoint['dType']))) {echo "selected=\"selected\"";} ?>>集貨站狗場</option>
                        </select>
                      </div>
                    ...
                    <div class="col-md-6">
                      <h5>狗場負責人</h5>
                      <input name="dManagement" type="text" required="required"  placeholder="輸入負責人" value="<?php echo $row_rs_findpoint['dManagement']; ?>">
                      </div>
                  ...
                  <div class="row with-forms">
                    ...
                    <div class="col-md-6">
                      <h5>毛小孩數量</h5>
                      <input name="dHavedog" type="number" required="required"  placeholder="輸入數量單位" value="<?php echo $row_rs_findpoint['dHavedog']; ?>">
              ...
              <div class="add-listing-section margin-top-25">
                ...
                <!-- Description -->
                <div class="form">
                  <h4>場所簡介<i class="tip" data-tip-content="參考格式標題28px,內容20px，圖片內容和標題可空一格(不一定要做標題)，圖片最後插入，設定右方的指令，再將圖片移動到第一格位置。"></i>→首張圖片class格式請設定：col-md-4 col-sm-12 res-margin2</h4>
                  <textarea name="dIntroduction" cols="40" rows="3" required class="WYSIWYG" id="summary" spellcheck="true"><?php echo $row_rs_findpoint['dIntroduction']; ?></textarea>
                ...
                <div class="row with-forms">
                  ...
                  <div class="col-md-4">
                    ...
                    <input name="dCphone" type="text" value="<?php echo $row_rs_findpoint['dCphone']; ?>">
                    </div>
                  
                  <!-- Website -->
                  <div class="col-md-4">
                    <h5>連絡家電<span>(選填)</span></h5>
                    <input name="dHphone" type="text" value="<?php echo $row_rs_findpoint['dHphone']; ?>">
                    </div>
                  
                  <!-- FB -->
                  <div class="col-md-4">
                    <h5 class="fb-input"><i class="fa fa-facebook-square"></i>Facebook專頁<span>(選填)</span></h5>
                    <input name="dFb" type="text" placeholder="e.g. https://www.facebook.com/abc" value="<?php echo $row_rs_findpoint['dFb']; ?>">
                    </div>
                  
                  <!-- YOUTUBE -->
                  <div class="col-md-12">
                    <h5>介紹影片連結<span>(請插入在簡介中，此處設定不生效)</span></h5>
                    <input name="dYoutube" type="text" value="<?php echo $row_rs_findpoint['dYoutube']; ?>">
                    </div>
              ...
              <div class="add-listing-section margin-top-25">
                ...
                <div class="submit-section">
                  ...
                  <div class="row with-forms">
                    ...
                    <div class="col-md-12">
                      <h5>地址(選填)</h5>
                      <input name="dMap" type="text" placeholder="e.g. 台中市新社區" value="<?php echo $row_rs_findpoint['dMap']; ?>">
                      </div>
                    
                    <!-- 座標 -->
                    <div class="col-md-12">
                      <h5>座標位置<i class="tip" data-tip-content="座標位置非公開地址，請先到GoogleMap標記誇張適合放的位置即可"></i></h5>
                      <input name="dMappoint" type="text" required placeholder="e.g. 24.248623,120.8372269" value="<?php echo $row_rs_findpoint['dMappoint']; ?>">
                      </div>
                    </div>
                  <!-- Row / End -->
              ...
              <div class="add-listing-section margin-top-45">
                ...
                <div class="col-md-5">
                  ...
                  <select id="selectthings" class="chosen-select" data-placeholder="請選擇" onChange='chg()'>
                    ...
                    <?php do { $basic[$row_rs_things['tId']] = $row_rs_things['tName']; ?>
                    <option value="<?php echo $row_rs_things['tId']; ?>"><?php echo $row_rs_things['tName']; ?></option>
  ...
  <table id="pricing-list-container2">
    <?php if ($totalRows_rs_basicneed > 0) { // Show if recordset not empty ?>
    <?php do { ?>
      <tr class="pricing-list-item pattern2">
        <td>
          <div class="fm-input pricing-name"><input name="nName[]" type="text" placeholder="項目" value="<?php echo $basic[$row_rs_basicneed['nLtid']] ?>" readonly /></div>
          <div class="fm-input pricing-ingredients"><input name="nRemarks[]" type="text" placeholder="備註" value="<?php echo $row_rs_basicneed['nRemarks']; ?>" /></div>
          <div class="fm-close"><a class="delete" href="#"><i class="fa fa-remove"></i></a></div>
          <input name="nLtid[]" type="hidden" id="nLtid" value="<?php echo $row_rs_basicneed['nLtid']; ?>">
        </td>
      </tr>
      <?php } while ($row_rs_basicneed = mysqli_fetch_assoc($rs_basicneed)); ?>
     <?php } // Show if recordset not empty ?>
		  ...
		  function chg() { /*選中新增*/
		   ...
		   var currSelectValue = document.getElementById('selectthings').value;

		  var currSelectText = document.getElementById('selectthings').options[document.getElementById('selectthings').selectedIndex].text;
			...
			newElem.appendTo('table#pricing-list-container2');
                      ...
                      <tr class="pricing-list-item pattern">
                        <td><div class="fm-move"><i class="sl sl-icon-cursor-move"></i></div>
                          <div class="fm-input pricing-name">
                            <input name="nName2[]" type="text" placeholder="項目" value="<?php echo $row_rs_othersneed['nName']; ?>" />
                          </div>
                          <div class="fm-input pricing-ingredients">
                            <input name="nRemarks2[]" type="text" placeholder="備註" value="<?php echo $row_rs_othersneed['nRemarks']; ?>" />
                          </div>
                        <div class="fm-close"><a class="delete" href="#"><i class="fa fa-remove"></i></a></div></td>
                      </tr>
                       <?php } while ($row_rs_othersneed = mysqli_fetch_assoc($rs_othersneed)); ?>
              ...
              <div class="add-listing-section margin-top-25">
                ...
                <!-- Headline -->
                <div class="add-listing-headline">
                  <h3><i class="sl sl-icon-picture"></i>代表照片(建議大小800*568，不須更改圖片此處跳過)
                    <input name="Filename" type="hidden" id="Filename" value="<?php echo $row_rs_findpoint['dPicture']; ?>">
                    </h3>
                  </div>
                ...
                <div class="submit-section">
                   ...
                   <div class="photoUpload">
							    ...
							    <input name="fileField" type="file" class="upload"/>
							</div>
...
<?php
mysqli_free_result($rs_findpoint);

mysqli_free_result($rs_basicneed);
...
mysqli_free_result($rs_othersneed);
?>

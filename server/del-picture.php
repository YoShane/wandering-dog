...
$colname_rs_delp = "-1";
if (isset($_GET['pId'])) {
  $colname_rs_delp = $_GET['pId'];
}

$query_rs_delp = sprintf("SELECT pNote,pName FROM pointalbum WHERE pId = %s", GetSQLValueString($Wandering,$colname_rs_delp, "int"));
...
if ((isset($_GET['pId'])) && ($_GET['pId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM pointalbum WHERE pId=%s",
                       GetSQLValueString($Wandering,$_GET['pId'], "int"));

 
  $Result1 = mysqli_query($Wandering, $deleteSQL) or die(mysqli_error());

 $updateGoTo = "show-pictures.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  
  @unlink("../images/uploads/thumb-" . $row_rs_delp['pName']);
   @unlink("../images/uploads/" . $row_rs_delp['pName']);
  
header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功刪除圖片" . $row_rs_delp['pNote'] . "成功')</script>"; 

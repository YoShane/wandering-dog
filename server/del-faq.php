...
$colname_rs_delf = "-1";
if (isset($_GET['fId'])) {
  $colname_rs_delf = $_GET['fId'];
}

$query_rs_delf = sprintf("SELECT fQuestion FROM faq WHERE fId = %s", GetSQLValueString($Wandering,$colname_rs_delf, "int"));
$rs_delf = mysqli_query($Wandering, $query_rs_delf) or die(mysqli_error());
$row_rs_delf = mysqli_fetch_assoc($rs_delf);
$totalRows_rs_delf = mysqli_num_rows($rs_delf);




if ((isset($_GET['fId'])) && ($_GET['fId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM faq WHERE fId=%s",
...
header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功刪除問答→" . $row_rs_delf['fQuestion'] . "成功')</script>"; 

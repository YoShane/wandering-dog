...
define("DESTINATION_FOLDER", "../images/repdog");
define("no_error", "");
define("yes_error", "");
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE repdog SET rOrder=%s, rName=%s, rPicture=%s, rGender=%s, rAge=%s, rIntroduction=%s WHERE rId=%s",
                       GetSQLValueString($Wandering,$_POST['rOrder'], "int"),
                       GetSQLValueString($Wandering,$_POST['rName'], "text"),
                       GetSQLValueString($Wandering,$_name_, "text"),
                       GetSQLValueString($Wandering,$_POST['rGender'], "int"),
                       GetSQLValueString($Wandering,$_POST['rAge'], "text"),
                       GetSQLValueString($Wandering,nl2br($_POST['rIntroduction']), "text"),
                       GetSQLValueString($Wandering,$_GET['rId'], "int"));

  
  $Result1 = mysqli_query($Wandering, $updateSQL) or die(mysqli_error());

  $updateGoTo = "show-repdog.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
    header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已完成編輯代表狗狗→" . $_POST['rName'] . "成功')</script>"; 
echo "<script>document.location.href='" . $updateGoTo . "';</script>";
}

$colname_rs_edrepdog = "-1";
if (isset($_GET['rId'])) {
  $colname_rs_edrepdog = $_GET['rId'];
}

$query_rs_edrepdog = sprintf("SELECT * FROM repdog WHERE rId = %s", GetSQLValueString($Wandering,$colname_rs_edrepdog, "int"));
$rs_edrepdog = mysqli_query($Wandering, $query_rs_edrepdog) or die(mysqli_error());
$row_rs_edrepdog = mysqli_fetch_assoc($rs_edrepdog);
$totalRows_rs_edrepdog = mysqli_num_rows($rs_edrepdog);
?>
...
================================================== -->
          ...
          <div id="add-listing">
            ...
            <div class="add-listing-section">
              ...
              <div class="add-listing-headline">
                <h3><i class="sl sl-icon-note"></i>狗狗基本資料</h3>
              </div>
              <!-- Title -->
              <div class="row with-forms">
                <div class="col-md-12">
                  <h5>狗狗名字</h5>
                  <input name="rName" type="text" required  class="search-field" placeholder="輸入稱呼" value="<?php echo $row_rs_edrepdog['rName']; ?>" />
                </div>
              </div>
                ...
                <div class="col-md-4">
									<h5>狗狗性別</h5>
									<select name="rGender" class="chosen-select-no-single" id="s" >
									  <option value="0" <?php if (!(strcmp(0, $row_rs_edrepdog['rGender']))) {echo "selected=\"selected\"";} ?>>female♀</option>
									  <option value="1" <?php if (!(strcmp(1, $row_rs_edrepdog['rGender']))) {echo "selected=\"selected\"";} ?>>male♂</option>
									  <option value="2" <?php if (!(strcmp(2, $row_rs_edrepdog['rGender']))) {echo "selected=\"selected\"";} ?>>other?</option>
                                    </select>
				  </div>
                                
                  <div class="col-md-4">
					<h5>大致年齡</h5>
							  <input name="rAge" type="text" required="required" id="rAge" value="<?php echo $row_rs_edrepdog['rAge']; ?>">
						 </div>
                            
                  <div class="col-md-4">
					<h5>排序<span>(選填)</span><i class="tip" data-tip-content="數字越小，優先度越高。"></i></h5>
							  <input name="rOrder" type="number" required="required" id="rOrder" min="0" value="<?php echo $row_rs_edrepdog['rOrder']; ?>">
						 </div>        
 ...
 <div class="col-md-12">
                  <h5>狗狗介紹&故事</h5>
                  <textarea name="rIntroduction" required class="search-field" id="rIntroduction"><?php echo br2nl($row_rs_edrepdog['rIntroduction']); ?></textarea>
</div>
          ...
          <!-- Headline -->
          <div class="add-listing-headline">
            <h3><i class="sl sl-icon-picture"></i>代表照片(建議大小600*600)</h3>
          </div>
          <!-- Dropzone -->
          <div class="submit-section">
             <div class="photoUpload">
							    <span><i class="sl sl-icon-arrow-up-circle"></i> 上傳狗狗照片</span>
							    <input name="fileField" type="file" class="upload" id="fileField"/>
                                <input name="Filename" type="hidden" id="Filename" value="<?php echo $row_rs_edrepdog['rPicture']; ?>">
                             							</div>
          </div>
...
<?php
mysqli_free_result($rs_edrepdog);
?>

...
$colname_rs_delo = "-1";
if (isset($_GET['oId'])) {
  $colname_rs_delo = $_GET['oId'];
}

$query_rs_delo = sprintf("SELECT * FROM ourteam WHERE oId = %s", GetSQLValueString($Wandering,$colname_rs_delo, "int"));
$rs_delo = mysqli_query($Wandering, $query_rs_delo) or die(mysqli_error());
$row_rs_delo = mysqli_fetch_assoc($rs_delo);
$totalRows_rs_delo = mysqli_num_rows($rs_delo);




if ((isset($_GET['oId'])) && ($_GET['oId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM ourteam WHERE oId=%s",
 ...
 $updateGoTo = "show-ourteam.php";
  ...
  @unlink("../images/team/" . $row_rs_delo['oPicture']);
  
header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功刪除成員資料→" . $row_rs_delo['oName'] . "成功')</script>"; 

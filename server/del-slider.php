...
$colname_rs_dels = "-1";
if (isset($_GET['sId'])) {
  $colname_rs_dels = $_GET['sId'];
}

$query_rs_dels = sprintf("SELECT * FROM slider WHERE sId = %s", GetSQLValueString($Wandering,$colname_rs_dels, "int"));
$rs_dels = mysqli_query($Wandering, $query_rs_dels) or die(mysqli_error());
$row_rs_dels = mysqli_fetch_assoc($rs_dels);
$totalRows_rs_dels = mysqli_num_rows($rs_dels);




if ((isset($_GET['sId'])) && ($_GET['sId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM slider WHERE sId=%s",
 ...
 $updateGoTo = "show-slider.php";
  ...
  @unlink("../images/" . $row_rs_dels['sPicture']);
  
header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功刪除滑動面板→" . $row_rs_dels['sTitle'] . "成功')</script>"; 

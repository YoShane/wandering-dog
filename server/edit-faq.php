...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE faq SET fQuestion=%s, fAnswer=%s WHERE fId=%s",
                       GetSQLValueString($Wandering,$_POST['fQuestion'], "text"),
                       GetSQLValueString($Wandering,$_POST['fAnswer'], "text"),
                       GetSQLValueString($Wandering,$_GET['fId'], "int"));

  
  $Result1 = mysqli_query($Wandering, $updateSQL) or die(mysqli_error());

  $updateGoTo = "show-faq.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已完成編輯問答→" . $_POST['fQuestion'] . "成功')</script>"; 
...
$colname_rs_myfaq = "-1";
if (isset($_GET['fId'])) {
  $colname_rs_myfaq = $_GET['fId'];
}

$query_rs_myfaq = sprintf("SELECT * FROM faq WHERE fId = %s", GetSQLValueString($Wandering,$colname_rs_myfaq, "int"));
$rs_myfaq = mysqli_query($Wandering, $query_rs_myfaq) or die(mysqli_error());
$row_rs_myfaq = mysqli_fetch_assoc($rs_myfaq);
$totalRows_rs_myfaq = mysqli_num_rows($rs_myfaq);
?>
<!DOCTYPE html> 
<html  lang="zh-TW">
<head>
...
================================================== -->
				...
				<div class="col-md-12">
					...
					<nav id="breadcrumbs">
						<ul>
							<li><a href="#">後臺管理</a></li>
							<li><a href="#">狗場管理</a></li>
							<li><a href="./show-dogpoint.php">編輯狗場列表</a></li>
                            <li><a href="./show-faq.php?dId=<?php echo $_GET['dId']; ?>">編輯FAQ列表</a></li>
                            <li>編輯FAQ</li>
						</ul>
					</nav>
				</div>
          ...
          <div id="add-listing">
            ...
            <div class="add-listing-section">
              ...
              <div class="row with-forms">
                <div class="col-md-12">
                  <h5>問題內容</h5>
                  <input name="fQuestion" type="text" required  class="search-field" placeholder="輸入問題" value="<?php echo $row_rs_myfaq['fQuestion']; ?>" />
                </div>
              </div>
              <!-- Row -->
              <div class="row with-forms">
 <div class="col-md-12">
                  <h5>回覆內容</h5>
                  <textarea name="fAnswer" required class="search-field" id="fAnswer"><?php echo $row_rs_myfaq['fAnswer']; ?></textarea>
</div>

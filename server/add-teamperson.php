...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO ourteam (oName, oWork, oWorkcontent, oPicture) VALUES (%s, %s, %s, %s)",
                       GetSQLValueString($Wandering,$_POST['oName'], "text"),
                       GetSQLValueString($Wandering,$_POST['oWork'], "text"),
                       GetSQLValueString($Wandering,$_POST['oWorkcontent'], "text"),
                       GetSQLValueString($Wandering,$_name_, "text"));

  
  $Result1 = mysqli_query($Wandering, $insertSQL) or die(mysqli_error());

  $insertGoTo = "show-ourteam.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
    $insertGoTo .= $_SERVER['QUERY_STRING'];
  }
   header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功新增成員→" . $_POST['oName'] . "成功')</script>"; 
...
================================================== -->
          ...
          <div id="add-listing">
            ...
            <div class="add-listing-section">
              ...
              <div class="row with-forms">
                <div class="col-md-12">
                  <h5>姓名</h5>
                  <input name="oName" type="text" required  class="search-field" placeholder="請在此輸入姓名" />
                </div>
                
                <div class="col-md-12">
                  <h5>工作職責</h5>
                  <input name="oWork" type="text" required  class="search-field" placeholder="請在此輸入工作職責" />
                </div>
              </div>
             <!-- Row -->
              <div class="row with-forms">
 <div class="col-md-12">
                  <h5>職責說明</h5>
                  <textarea name="oWorkcontent" class="search-field" id="oWorkcontent" placeholder="輸入職責說明"></textarea>
</div>

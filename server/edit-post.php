...
define("DESTINATION_FOLDER", "../images/post");
define("no_error", "");
define("yes_error", "");
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE post SET pTitle=%s, pPicture=%s, pContent=%s, pType=%s, pShow=%s, pOutline=%s WHERE pId=%s",
                       GetSQLValueString($Wandering,$_POST['pTitle'], "text"),
                       GetSQLValueString($Wandering,$_name_, "text"),
                       GetSQLValueString($Wandering,$_POST['pContent'], "text"),
                       GetSQLValueString($Wandering,$_POST['pType'], "int"),
                       GetSQLValueString($Wandering,$_POST['pShow'], "int"),
                       GetSQLValueString($Wandering,nl2br($_POST['pOutline']), "text"),
                       GetSQLValueString($Wandering,$_GET['pId'], "int"));

  
  $Result1 = mysqli_query($Wandering, $updateSQL) or die(mysqli_error());

  $updateGoTo = "show-post.php";
  if (isset($_SERVER['QUERY_STRING'])) {
  ...
  header ('Content-type: text/html; charset=utf-8');
echo "<script>javascript:alert('已成功編輯貼文→" . $_POST['pTitle'] . "成功')</script>"; 
echo "<script>document.location.href='" . $updateGoTo . "';</script>";
}

$colname_rs_findpost = "-1";
if (isset($_GET['pId'])) {
  $colname_rs_findpost = $_GET['pId'];
}

$query_rs_findpost = sprintf("SELECT * FROM post WHERE pId = %s", GetSQLValueString($Wandering,$colname_rs_findpost, "int"));
$rs_findpost = mysqli_query($Wandering, $query_rs_findpost) or die(mysqli_error());
$row_rs_findpost = mysqli_fetch_assoc($rs_findpost);
$totalRows_rs_findpost = mysqli_num_rows($rs_findpost);
?>
          ...
          <div id="add-listing">
            ...
            <div class="add-listing-section">
              ...
              <div class="add-listing-headline">
                <h3><i class="sl sl-icon-doc"></i>基本設定</h3>
              </div>
              <!-- Title -->
              <div class="row with-forms">
                <div class="col-md-12">
                  <h5>貼文標題</h5>
                  <input name="pTitle" type="text" required  class="search-field" placeholder="請在此輸入標題" value="<?php echo $row_rs_findpost['pTitle']; ?>" />
                </div>
              </div>
             <!-- Row -->
              <div class="row with-forms">
 <div class="col-md-12">
                  <h5>貼文大綱(為空大綱會顯示內容內容)</h5>
                  <textarea name="pOutline" class="search-field" id="pOutline" placeholder="輸入貼文大綱"><?php echo br2nl($row_rs_findpost['pOutline']); ?></textarea>
</div>
          ...
          <div class="add-listing-section margin-top-25">
            <!-- Headline -->
            <div class="add-listing-headline">
              <h3><i class="sl sl-icon-docs"></i>文章內文(Youtube連結請務必設為自適[Responsive]模式)</h3>
            </div>
            <!-- Description -->
            <div class="form">
 
              <textarea name="pContent" cols="40" rows="3" required class="WYSIWYG" id="summary" spellcheck="true"><?php echo $row_rs_findpost['pContent']; ?></textarea>
          ...
          <div class="add-listing-section margin-top-45">
            <!-- Headline -->
            <div class="add-listing-headline">
              <h3><i class="sl sl-icon-settings"></i>貼文設定</h3>
            </div>
            ...
            <div class="row with-forms">
              ...
              <div class="col-md-4">
									  <h5 class="margin-top-30 margin-bottom-10">貼文類型</h5>
									  <select name="pType" required class="chosen-select" id="selectthings">
									    <option value="0" selected="selected" <?php if (!(strcmp(0, $row_rs_findpost['pType']))) {echo "selected=\"selected\"";} ?>>消息</option>
									    <option value="1" <?php if (!(strcmp(1, $row_rs_findpost['pType']))) {echo "selected=\"selected\"";} ?>>募集</option>
									    <option value="2" <?php if (!(strcmp(2, $row_rs_findpost['pType']))) {echo "selected=\"selected\"";} ?>>活動</option>
									    <option value="3" <?php if (!(strcmp(3, $row_rs_findpost['pType']))) {echo "selected=\"selected\"";} ?>>系統</option>
									    <option value="4" <?php if (!(strcmp(4, $row_rs_findpost['pType']))) {echo "selected=\"selected\"";} ?>>綜合</option>
									    </select>
									  
								</div>
                         
                           <div class="col-md-8">
									  <h5 class="margin-top-30 margin-bottom-10">貼文顯示標題圖片</h5>
									  <select name="pShow" required class="chosen-select" id="selectthings">
									    <option value="0" selected="selected" <?php if (!(strcmp(0, $row_rs_findpost['pShow']))) {echo "selected=\"selected\"";} ?>>顯示</option>
									    <option value="1" <?php if (!(strcmp(1, $row_rs_findpost['pShow']))) {echo "selected=\"selected\"";} ?>>不顯示</option>
									    </select>
        ...
        <div class="add-listing-section margin-top-25">
          <!-- Headline -->
          <div class="add-listing-headline">
            <h3><i class="sl sl-icon-picture"></i>標題照片(建議大小900*500)</h3>
          </div>
          <!-- Dropzone -->
          <div class="submit-section">
             <div class="photoUpload">
                                ...
                                <input name="Filename" type="hidden" id="Filename" value="<?php echo $row_rs_findpost['pPicture']; ?>">
							</div>

...
$maxRows_rs_findpost = 5;
$pageNum_rs_findpost = 0;
if (isset($_GET['pageNum_rs_findpost'])) {
  $pageNum_rs_findpost = $_GET['pageNum_rs_findpost'];
}
$startRow_rs_findpost = $pageNum_rs_findpost * $maxRows_rs_findpost;

$colname_rs_findpost = "-1";
if (isset($_GET['keyword'])) {
  $colname_rs_findpost = $_GET['keyword'];
 
}

$query_rs_findpost = sprintf("SELECT * FROM post WHERE pTitle LIKE %s OR pContent LIKE %s OR pOutline LIKE %s ORDER BY pStime DESC", GetSQLValueString($Wandering,"%" . $colname_rs_findpost . "%", "text"), GetSQLValueString($Wandering,"%" . $colname_rs_findpost . "%", "text"), GetSQLValueString($Wandering,"%" . $colname_rs_findpost . "%", "text"));
$query_limit_rs_findpost = sprintf("%s LIMIT %d, %d", $query_rs_findpost, $startRow_rs_findpost, $maxRows_rs_findpost);
$rs_findpost = mysqli_query($Wandering, $query_limit_rs_findpost) or die(mysqli_error());
$row_rs_findpost = mysqli_fetch_assoc($rs_findpost);

if (isset($_GET['totalRows_rs_findpost'])) {
  $totalRows_rs_findpost = $_GET['totalRows_rs_findpost'];
} else {
  $all_rs_findpost = mysqli_query($Wandering, $query_rs_findpost);
  $totalRows_rs_findpost = mysqli_num_rows($all_rs_findpost);
}
$totalPages_rs_findpost = ceil($totalRows_rs_findpost/$maxRows_rs_findpost)-1;
...
{
                GLOBAL $maxRows_rs_finshpost,$totalRows_rs_finshpost;
		...
		if($totalPages_Recordset1 >= 1) {
			...
			if(!empty($_GET) || !empty($HTTP_GET_VARS)){
				...
				foreach ($_GET as $_get_name => $_get_value) {
					if ($_get_name != "pageNum_rs_finshpost") {
			...
			$firstArray = ($pageNum_Recordset1 > 0) ? "<a href=\"$_SERVER[PHP_SELF]?pageNum_rs_finshpost=$precedente$_get_vars$_SESSION[listURL]\" class='pagenate-prev fa fa-angle-left' rel='prev'></a>" :  "$prev_Recordset1";
			//頁碼
			for($a = $fgp+1; $a <= $egp; $a++){
				$theNext = $a-1;
				if($show_page)
				{
					...
					$min_l = (($a-1)*$maxRows_rs_finshpost) + 1;
					$max_l = ($a*$maxRows_rs_finshpost >= $totalRows_rs_finshpost) ? $totalRows_rs_finshpost : ($a*$maxRows_rs_finshpost);
					$textLink = "$min_l - $max_l";
				}
				$_ss_k = floor($theNext/26);
				if ($theNext != $pageNum_Recordset1)
				{
					$pagesArray .= "<li><a href=\"$_SERVER[PHP_SELF]?pageNum_rs_finshpost=$theNext$_get_vars$_SESSION[listURL]\">";
			...
			$lastArray = ($pageNum_Recordset1 < $totalPages_Recordset1) ? "<a href=\"$_SERVER[PHP_SELF]?pageNum_rs_finshpost=$successivo$_get_vars$_SESSION[listURL]\" class='pagenate-next fa fa-angle-right' rel='next'></a>" : "$next_Recordset1";
		}
	}
...
$okpoint = 0;
 ...
 if($totalRows_rs_findpost > 0){
	

	
    do { 
          $tempNum = $row_rs_findpost['pId']; //第一次篩選結果
	       $check = false;
		   
		   
		    foreach ($newstr as $value){
			     
				 if($value == $row_rs_findpost['pType']){
					 
					 /* echo "<script>javascript:alert('未分類取得->" . $tempNum . "條件" . $value . "遇到->" . $row_rs_findpost['pType'] . "')</script>"; */
					  $check = true;
					  break;
	...
	$okpoint++;
	    ...
	    }while ($row_rs_findpost = mysqli_fetch_assoc($rs_findpost));
	

mysqli_free_result($rs_findpost);
...
$count = 0;
	...
	if($stopnum != 0){
$maxRows_rs_finshpost = 5;
$pageNum_rs_finshpost = 0;
if (isset($_GET['pageNum_rs_finshpost'])) {
  $pageNum_rs_finshpost = $_GET['pageNum_rs_finshpost'];
}
$startRow_rs_finshpost = $pageNum_rs_finshpost * $maxRows_rs_finshpost;
	
			

$query_rs_finshpost = "SELECT * FROM post WHERE ";
foreach ($okid as $value){ $count++;
$query_rs_finshpost = $query_rs_finshpost . "pId = " . $value . " ";
if($stopnum != $count ) {
	$query_rs_finshpost = $query_rs_finshpost . "OR ";
   }
}
/* echo "<script>javascript:alert('未分類取得->" . $query_rs_finshpost . "')</script>"; */
$query_limit_rs_finshpost = sprintf("%s LIMIT %d, %d", $query_rs_finshpost, $startRow_rs_finshpost, $maxRows_rs_finshpost);
$rs_finshpost = mysqli_query($Wandering, $query_limit_rs_finshpost) or die(mysqli_error());
$row_rs_finshpost = mysqli_fetch_assoc($rs_finshpost);

if (isset($_GET['totalRows_rs_finshpost'])) {
  $totalRows_rs_finshpost = $_GET['totalRows_rs_finshpost'];
} else {
  $all_rs_finshpost = mysqli_query($Wandering, $query_rs_finshpost);
  $totalRows_rs_finshpost = mysqli_num_rows($all_rs_finshpost);
}
$totalPages_rs_finshpost = ceil($totalRows_rs_finshpost/$maxRows_rs_finshpost)-1;
	}
...
<div class="container">
	...
	<div class="row">
        ...
        <?php if ($totalRows_rs_finshpost > 0) { // Show if recordset not empty ?>
		  ...
		  <?php do { ?>
		    ...
		    <div class="blog-post">
		      ...
		      <!-- Img -->
		      <a href="pages-blog-post.php?Id=<?php echo $row_rs_finshpost['pId']; ?>" class="post-img">
		        <img src="images/post/<?php echo $row_rs_finshpost['pPicture']; ?>" alt="">
	          </a>
		      
		      <!-- Content -->
		      <div class="post-content">
		        <h3><a href="pages-blog-post.php?Id=<?php echo $row_rs_finshpost['pId']; ?>"><?php echo $row_rs_finshpost['pTitle']; ?></a></h3>
		        
		        <ul class="post-meta">
		          <li><?php echo date('Y 年 m 月 d 日',strtotime($row_rs_finshpost['pStime'])); ?></li>
		          <li><a href="#"><?php if($row_rs_finshpost['pType'] == 0){ echo "消息";}
elseif($row_rs_finshpost['pType'] == 1){ echo "募集";}
elseif($row_rs_finshpost['pType'] == 2){ echo "活動";}
elseif($row_rs_finshpost['pType'] == 3){ echo "系統";}
elseif($row_rs_finshpost['pType'] == 4){ echo "綜合";} ?></a></li>
		          
		          </ul>
		        
		        <p><?php if($row_rs_finshpost['pOutline'] != ""){ echo $row_rs_finshpost['pOutline'];}
					else{ echo TrimByLength2($row_rs_finshpost['pContent'], 50, false);} ?></p>
		        
		        <a href="pages-blog-post.php?Id=<?php echo $row_rs_finshpost['pId']; ?>" class="read-more">詳細內容<i class="fa fa-angle-right"></i></a>
		        </div>
		      
		      </div>
		    <!-- Blog Post / End -->
		    <?php } while ($row_rs_finshpost = mysqli_fetch_assoc($rs_finshpost)); ?>
...
$separator = " ";
$max_links = 5;
$pages_navigation_rs_finshpost = buildNavigation($pageNum_rs_finshpost,$totalPages_rs_finshpost,$prev_rs_finshpost,$next_rs_finshpost,$separator,$max_links,true); 

print $pages_navigation_rs_finshpost[0]; ?>
 <?php print $pages_navigation_rs_finshpost[2]; ?>
 <ul class="pagenate-body"> 
 <?php print $pages_navigation_rs_finshpost[1]; ?>
 </ul>
...
<?php }else{ ?>
<img style="margin-top:-50px;" src="images/no_info.png" alt=""/>
<?php } ?>
		</div>
...
for (i = 0; i < field.length; i++)
  ...
  <input name="keyword" type="text" id="search" placeholder="請輸入關鍵字" value="<?php echo $_GET['keyword']; ?>">
  <button type="submit">
                ...
                <!-- Panel Dropdown -->
						<div class="panel-dropdown wide float-left">
							...
							<div class="panel-dropdown-content checkboxes">                           
								...
								<div class="row">
								    <div class="col-md-3">
									    <input <?php if (in_array("0",$newstr)) {echo "checked=\"checked\"";} ?> name="list" type="checkbox" id="check-0" value="0">
									    <label for="check-0">消息</label>

								      </div>

									<div class="col-md-3">
									    <input <?php if (in_array("1",$newstr)) {echo "checked=\"checked\"";} ?>  name="list" type="checkbox" id="check-1" value="1">
                                  ...
                                  <div class="col-md-3">
                              		...
                              		<div class="row">
								    <div class="col-md-3">
									    <input <?php if (in_array("2",$newstr)) {echo "checked=\"checked\"";} ?>  name="list" type="checkbox" id="check-2" value="2">
									    <label for="check-2">活動</label>

								      </div>

									<div class="col-md-3">
									    <input <?php if (in_array("3",$newstr)) {echo "checked=\"checked\"";} ?>  name="list" type="checkbox" id="check-3" value="3">
                                  ...
                                  <div class="col-md-3">
                              		...
                              		<div class="row">
								    <div class="col-md-3">
									    <input <?php if (in_array("4",$newstr)) {echo "checked=\"checked\"";} ?>  name="list" type="checkbox" id="check-4" value="4">
...
mysqli_free_result($rs_finshpost);
?>

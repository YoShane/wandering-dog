...
$maxRows_rs_dogpoint = 6;
$pageNum_rs_dogpoint = 0;
...
$query_rs_dogpoint = "SELECT * FROM dogpoint";
$query_limit_rs_dogpoint = sprintf("%s LIMIT %d, %d", $query_rs_dogpoint, $startRow_rs_dogpoint, $maxRows_rs_dogpoint);
$rs_dogpoint = mysqli_query($Wandering, $query_limit_rs_dogpoint) or die(mysqli_error());
$row_rs_dogpoint = mysqli_fetch_assoc($rs_dogpoint);

if (isset($_GET['totalRows_rs_dogpoint'])) {
  $totalRows_rs_dogpoint = $_GET['totalRows_rs_dogpoint'];
} else {
  $all_rs_dogpoint = mysqli_query($Wandering, $query_rs_dogpoint);
  $totalRows_rs_dogpoint = mysqli_num_rows($all_rs_dogpoint);
}
$totalPages_rs_dogpoint = ceil($totalRows_rs_dogpoint/$maxRows_rs_dogpoint)-1;
		...
		<div class="col-md-12">
			<div class="main-search-input gray-style margin-top-0 margin-bottom-10">
				...
				<div class="main-search-input-item">
					<input name="Keyword" type="text" id="Keyword" placeholder="輸入關鍵字(可留空，只篩選條件)" value=""/>
				</div>
 ...
 ?>
								...
								<div class="row">
                                    ...
                                    <?php  for($i=1; $i <= $oneneed; $i++){ ?>
									    <input name="list" type="checkbox" id="check-<?php echo $i; ?>" value="<?php echo $tNum[$i]; ?>" checked="checked">
				...
				<center><button class="button">搜尋</button></center>
			</div>
		</div>
		<!-- Search Section / End -->
</form>
			...
			<div class="row">
			    ...
			    <!-- Listing Item -->
				  <div class="col-lg-4 col-md-6">
				    <a href="Introduction.php?Id=<?php echo $row_rs_dogpoint['dId']; ?>" class="listing-item-container compact">
				      <div class="listing-item">
				        <img src="images/dogpoint/<?php echo $row_rs_dogpoint['dPicture']; ?>" alt="">
                        
				        <div class="listing-item-details">
								<ul>
									<li><?php echo $row_rs_dogpoint['dName']; ?></li>
								</ul>
							</div>
                            
                        <?php if($row_rs_dogpoint['dType'] == 1000){ ?>
				        <div class="listing-badge now-open">物資集貨站</div>
                        <?php } ?>
				        
				        <div class="listing-item-content">
				          <h3>負責人：<?php echo $row_rs_dogpoint['dManagement']; ?></h3>
				          <span>收留約<?php echo $row_rs_dogpoint['dHavedog']; ?>隻狗狗</span>
				          </div>
